...
1. A series of error messages recording all errors found in your code (in stdout). Files will stop being created the moment an error is detected (for example- if an error is detected in the pre-assember, than no files will be created, but if an error is detected in the first pass than an .am files will be created but no more files after that)
